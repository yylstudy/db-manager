...
public class BackupConfigServiceImpl extends ServiceImpl<BackupConfigMapper, BackupConfig> implements BackupConfigService {
    ...
    public Result active(Long id) {
        ...
        Result result = checkSoftInstall(backupConfig);
    ...
    public void checkSoftInstallResult(BackupConfig backupConfig){
        Result result = checkSoftInstall(backupConfig);
    ...
    public Result recoverPrepare(BackupConfig backupConfig) {
        ...
        if(CommonConstant.LOCALHOST_FILE.equals(backupConfig.getIslocalhostFile())){
            backupConfig = this.getById(backupConfig.getId());
            setSsh(backupConfig);
    ...
    public BackupResult doBackup(BackupConfig backupConfig,boolean checkTime) {
        ...
        try{
            DatasourceProp prop = datasourcePropService.getById(backupConfig.getPropId());
            checkSoftInstallResult(backupConfig);
            session = SshUtil.getSession(prop.getIp(),prop.getSshPort(),
                    prop.getSshUser(),PwdUtil.decryptPwd(prop.getSshPassword()));
            Session finalSession = session;
            ...
            SshUtil.doExecute(session, channelSftp -> {
                ...
                for(ChannelSftp.LsEntry entry:list){
                    ...
                    if(fileName.contains(fullPrefix)){
                        maxTimeFullFile = getMaxTimeFile(maxTimeFullFile,fileName);
                    }
                    if(fileName.contains(incrPrefix)||fileName.contains(fullPrefix)){
                        maxTimeFile = getMaxTimeFile(maxTimeFile,fileName);
                    }
                ...
                StringBuilder sb = new StringBuilder("xtrabackup ");
                Date date = new Date();
...
//                    SshUtil.deleteFile(channelSftp,newBackFileName);
    ...
    private String getMaxTimeFile(String maxTimeFile,String fileName){
