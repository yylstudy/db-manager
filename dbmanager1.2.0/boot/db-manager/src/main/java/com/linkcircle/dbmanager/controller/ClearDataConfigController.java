    ...
    private void checkIbdfrm(ClearDataConfig clearDataConfig){
        ...
        try{
            ...
            if(CommonConstant.DATASOURCE_K8S.equals(datasourceProp.getIsk8s())){
                ...
                session = SshUtil.getSession(k8sConfig.getNfsSshIp(),k8sConfig.getNfsSshPort(),
                        k8sConfig.getNfsSshUser(), k8sConfig.getNfsSshPassword());
            }else{
            ...
            if(execStatus==null||execStatus!=0){
                throw new BusinessException("ibdfrm存储目录"+ibdFrmDir+"不存在，请检查配置");
            }
        }finally {
            if(session!=null){
                session.disconnect();
