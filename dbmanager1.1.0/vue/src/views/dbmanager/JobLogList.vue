  ...
  <a-card :bordered="false">
    <div class="table-page-search-wrapper">
      <a-form-model layout="inline" :model="queryParam">
        <a-row :gutter="10">
          <a-col :md="6" :sm="12">
            <a-form-model-item label="执行开始时间"  style="margin-left:8px">
              <a-date-picker v-model="queryParam.queryDateStart" format="YYYY-MM-DD HH:MM:SS" valueFormat="YYYY-MM-DD HH:MM:SS" placeholder="请选择执行开始时间" />
            </a-form-model-item>
          </a-col>
          <a-col :md="6" :sm="12">
            <a-form-model-item label="执行结束时间"  style="margin-left:8px">
              <a-date-picker v-model="queryParam.queryDateEnd" format="YYYY-MM-DD HH:MM:SS" valueFormat="YYYY-MM-DD HH:MM:SS" placeholder="请选择执行结束时间" />
            </a-form-model-item>
          </a-col>
          <a-col :md="6" :sm="12">
...
<!--             <a-button type="primary" @click="handleAdd" icon="plus" style="margin-left: 18px">新增</a-button>-->
             <a-button type="primary" @click="searchQuery" icon="search" style="margin-left: 18px">查询</a-button>
              <a-button type="primary" @click="exportExcel" icon="search" style="margin-left: 18px">导出</a-button>
      ...
      <a-table
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <a-divider type="vertical"  v-show="record.taskType=='2'&&record.handleCode=='2'&&record.ibdfrmPath!=null&&record.ibdfrmPath!=''"/>
          <a @click="showIbdAndFrm(record)" v-show="record.taskType=='2'&&record.handleCode=='2'&&record.ibdfrmPath!=null&&record.ibdfrmPath!=''">查看ibd、frm</a>
  ...
  export default {
    ...
    methods: {
      ...
      exportExcel(){
        ...
        downFile("/joblog/exportExcel",params).then((data)=>{
          let url = window.URL.createObjectURL(new Blob([data],{type: 'application/vnd.ms-excel'}))
          let link = document.createElement('a')
          link.style.display = 'none'
          link.href = url
          link.setAttribute('download', '执行日志.xls')
