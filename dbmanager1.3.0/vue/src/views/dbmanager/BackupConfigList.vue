<template>
  <a-card :bordered="false">
    ...
    <a-modal
      ...
      :footer="null"
    >
      <p v-show="xtrabackupSuccess=='true' && !isK8s" style="color:green">已安装xtrabackup</p>
      <p v-show="xtrabackupSuccess=='false' && !isK8s"><a @click="installXtrabackup(dataJson)"  style="color:red">未安装xtrabackup，点击安装</a></p>
      <p v-show="xtrabackupSuccess=='ing' && !isK8s" style="color:red">正在安装xtrabackup，请勿关闭页面</p>
      <p v-show="qpressSuccess=='true'" style="color:green">已安装qpress</p>
      <p v-show="qpressSuccess=='false'"><a @click="installQpress(dataJson)"  style="color:red">未安装qpress，点击安装</a></p>
      <p v-show="qpressSuccess=='ing'" style="color:red">正在安装qpress，请勿关闭页面</p>
      <p v-show="podExistsSuccess && isK8s" style="color:green">mysql pod存在</p>
      <p v-show="!podExistsSuccess && isK8s" style="color:green">mysql pod不存在</p>
      <p v-show="dirSuccess" style="color:green">备份目录校验成功</p>
      <p v-show="!dirSuccess"style="color:#ff0000">备份目录不存在，请先创建备份目录</p>
      <p v-show="connectionSuccess" style="color:green">mysql配置成功</p>
      <p v-show="!connectionSuccess" style="color:red">mysql配置错误，请检查</p>
      <p v-show="enableRemoteStore&&expectSuccess"  style="color:green">已安装expect</p>
      <p v-show="enableRemoteStore&&!expectSuccess" style="color:red">未安装expect</p>
    ...
    methods: {
      ...
      checkSoft(record){
        ...
        httpAction("/backup/checkSoftInstall?id="+record.id,{},"post").then((res)=>{
          if(res.success){
            that.isK8s = res.result[0]
            that.showInstall = true;
            if(res.result[0]){
              that.podExistsSuccess = res.result[1]
              that.qpressSuccess = ''+res.result[2]
              that.dirSuccess = res.result[3]
              that.connectionSuccess = res.result[4]
            }else{
              that.xtrabackupSuccess = ''+res.result[1]
              that.qpressSuccess = ''+res.result[2]
              that.dirSuccess = res.result[3]
              that.connectionSuccess = res.result[4]
              that.expectSuccess = res.result[5]
            }
            that.searchQuery();
          }else{
            that.$message.warning(res.message);
        ...
        httpAction("/datasource/getById?id="+record.propId,{},"get").then((res)=>{
          if(res.success){
            that.enableRemoteStore = res.result.enableRemoteStore==1
          }else{
            that.$message.warning(res.message);
          }
        });
