...
update datasource_prop set `sudo_user`='0';
...
file_size bigint(18) COMMENT '文件大小',
raw_file_name VARCHAR(100) COMMENT '文件名称',
full_path VARCHAR(500) COMMENT '文件地址',
business_id VARCHAR(50) COMMENT '业务ID',
business_type VARCHAR(50) COMMENT '业务类型',
create_date datetime COMMENT '创建时间',
INDEX attachment_business (business_id,business_type)
)comment='附件表';

CREATE TABLE `k8s_config`  (
  `id` bigint(20) NOT NULL ,
  `name` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL COMMENT '集群名称',
  create_time datetime,
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci ROW_FORMAT = Dynamic;

alter table datasource_prop add isk8s varchar(10) COMMENT '是否k8s 0不是，1是';
alter table datasource_prop add k8s_config_id bigint(20) COMMENT 'k8s配置ID';
alter table datasource_prop add namespace varchar(50) COMMENT '命名空间';
alter table datasource_prop add pod_name varchar(50) COMMENT 'pod名称';
update datasource_prop set isk8s='0';

INSERT INTO `sys_permission`(`id`, `parent_id`, `name`, `url`, `component`, `component_name`, `redirect`, `menu_type`, `perms`, `perms_type`, `sort_no`, `always_show`, `icon`, `is_route`, `is_leaf`, `keep_alive`, `hidden`, `description`, `create_by`, `create_time`, `update_by`, `update_time`, `del_flag`, `rule_flag`, `status`, `internal_or_external`) VALUES ('1571778656027389954', '1437966560366755841', 'k8s配置', '/dbmanager/K8sConfigList', 'dbmanager/K8sConfigList', NULL, NULL, 1, NULL, '1', 1.00, 0, NULL, 1, 1, 0, 0, NULL, 'yyl', '2022-09-19 16:30:08', NULL, NULL, 0, 0, '1', 0);

INSERT INTO `sys_dict`(`id`, `dict_name`, `dict_code`, `description`, `del_flag`, `create_by`, `create_time`, `update_by`, `update_time`, `type`) VALUES ('1571786725440405506', '是否k8s', 'is_k8s', '', 0, 'yyl', '2022-09-19 17:02:12', NULL, NULL, 0);
INSERT INTO `sys_dict_item`(`id`, `dict_id`, `item_text`, `item_value`, `description`, `sort_order`, `status`, `create_by`, `create_time`, `update_by`, `update_time`) VALUES ('1571786796588384257', '1571786725440405506', '否', '0', '', 1, 1, 'yyl', '2022-09-19 17:02:29', NULL, NULL);
INSERT INTO `sys_dict_item`(`id`, `dict_id`, `item_text`, `item_value`, `description`, `sort_order`, `status`, `create_by`, `create_time`, `update_by`, `update_time`) VALUES ('1571786820676272129', '1571786725440405506', '是', '1', '', 1, 1, 'yyl', '2022-09-19 17:02:35', NULL, NULL);
drop view v_datasource_prop;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `v_datasource_prop` AS select `a`.`id` AS `id`,concat(`a`.`ip`,':',a.port,'(',`c`.`name`,'-',`b`.`project_name`,'-',`b`.`business_name`,')') AS `name` from ((`datasource_prop` `a` join `business` `b`) join `computer_room` `c`) where ((`a`.`business_id` = `b`.`id`) and (`a`.`computer_room_id` = `c`.`id`));


alter table computer_room add enable_remote_store TINYINT(1) COMMENT '是否开启远程存储，0否，1是';
alter table computer_room add ssh_ip varchar(50) COMMENT 'ssh 地址';
alter table computer_room add ssh_port varchar(50) COMMENT 'ssh端口';
alter table computer_room add ssh_username varchar(50) COMMENT 'ssh用户名';
alter table computer_room add ssh_password varchar(100) COMMENT 'ssh密码';
alter table computer_room add base_path varchar(100) COMMENT '基础目录';
alter table datasource_prop add enable_remote_store TINYINT(1) default 0 COMMENT '是否开启远程存储，0否，1是，默认不开启';

INSERT INTO `sys_dict`(`id`, `dict_name`, `dict_code`, `description`, `del_flag`, `create_by`, `create_time`, `update_by`, `update_time`, `type`) VALUES ('1788099949196517378', '是都开启远程存储', 'enable_remote_store', '', 0, 'yyl', '2024-05-08 14:53:44', NULL, NULL, 0);
