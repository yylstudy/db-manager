...
//            mysqlUserService.saveBatch(addList);
    ...
    public Result<?> getMysqlUserByLevel() {
        ...
        PageDTO<DatasourceProp> page = new PageDTO();
        page.setSize(10000);
        DatasourceProp datasourceProp = new DatasourceProp();
        List<DatasourceProp> datasourceProps = this.queryPage(page,datasourceProp).getRecords();
        for(DatasourceProp prop:datasourceProps){
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("label",prop.getComputerRoomName()+"-"+prop.getBusinessName()+"-"+prop.getIp());
