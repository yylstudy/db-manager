...
public class JobLogController {
    ...
    public ModelAndView exportExcel(JobLog jobLog) {
        Page<JobLog> page = new Page<JobLog>(1, 1000000);
        List<JobLog> jobLogList = jobLogService.queryPage(page, jobLog).getRecords();
        ModelAndView mv = new ModelAndView(new JeecgEntityExcelView());
        mv.addObject(NormalExcelConstants.FILE_NAME, "执行日志报表");
        mv.addObject(NormalExcelConstants.CLASS, JobLog.class);
        ExportParams exportParams=new ExportParams(null, null, "执行日志");
        exportParams.setImageBasePath(upLoadPath);
        mv.addObject(NormalExcelConstants.PARAMS,exportParams);
        mv.addObject(NormalExcelConstants.DATA_LIST, jobLogList);
        return mv;
    }
